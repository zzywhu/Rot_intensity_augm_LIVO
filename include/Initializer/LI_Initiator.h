...
#define LI_INITIATOR_API __declspec(dllexport)
#else
#define LI_INITIATOR_API __declspec(dllimport)
#endif
#else
#define LI_INITIATOR_API
#endif
...
struct LI_INITIATOR_API CalibState
{
    M3D rot_end;
    V3D ang_vel;
    V3D linear_vel;
...
class LI_INITIATOR_API LI_Init
{
public:
    EIGEN_MAKE_ALIGNED_OPERATOR_NEW
    std::ofstream fout_LiDAR_meas, fout_IMU_meas, fout_before_filt_IMU, fout_before_filt_Lidar, fout_acc_cost, fout_after_rot;
    ...
    void push_ALL_IMU_CalibState(const SensorMsgs::ImuData::ConstPtr &msg, const double &mean_acc_norm);
    ...
    void zero_phase_filt(const std::deque<CalibState> &signal_in, std::deque<CalibState> &signal_out);
    ...
    void outputPlot(const std::string& filePath);

    void outputErrorPlot(const std::string& filePath);
