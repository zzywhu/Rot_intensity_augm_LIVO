...
#define CONTOUR_MNG_API __declspec(dllexport)
#else
#define CONTOUR_MNG_API __declspec(dllimport)
#endif // __DLL_EXPORTS__
#else
#define CONTOUR_MNG_API
...
//  static constexpr size_t SizeAtCompileTime = sz;  // undefined reference when linking
    ...
    KeyFloatType &operator()(size_t i)
    { return array[i]; }

    const KeyFloatType &operator()(size_t i) const
    { return array[i]; }
...
class CONTOUR_MNG_API ContourManager
{
    // configuration
    ...
    // data
    std::vector <std::vector<std::shared_ptr < ContourView>>>
    cont_views_;  // TODO: use a parallel vec of vec for points?
...
//    bev_pixfs_.reserve(int(cfg_.n_col_ * cfg_.n_row_ * 0.05));
    ...
    /// \param ptr_gapc
    /// \param str_id
    void makeBEV(typename pcl::PointCloud<PointType>::Ptr &ptr_gapc, std::string str_id = "")
    {  //
        CHECK(ptr_gapc);
...
//    std::cout << "bev size byte: " << sizeInBytes<< std::endl;
    ...
    //    CHECK_GT(5.0, cfg_.lv_grads_.back());
        ...
        std::string dir = std::string(ROOT_DIR) + "results/bev_img/";
...
//              }
                            ...
                            if (dist < roi_radius - 1e-2 && bev_(rr, cc) > cfg_.lv_grads_[DIST_BIN_LAYERS[0]])
                            { // imprv key variance
...
//          key(2) = (cont_views_[ll][seq]->pos_mean_ - cont_views_[ll][seq]->com_).norm();
                    ...
                    key(0) =std::sqrt(cont_views_[ll][seq]->eig_vals_(1) * cont_views_[ll][seq]->cell_cnt_);  // max eigen value * cnt
                    key(1) =std::sqrt(cont_views_[ll][seq]->eig_vals_(0) * cont_views_[ll][seq]->cell_cnt_);  // min eigen value * cnt
//          key(2) = (cont_views_[ll][seq]->pos_mean_ - cont_views_[ll][seq]->com_).norm() *
...
//    }
        ...
        std::string fpath = std::string(ROOT_DIR) + "results/contours_orig-" + str_id_ + ".txt";
...
//    }
    ...
    void saveAccumulatedContours(int top_n) const
    {
        ...
        std::string fpath = std::string(ROOT_DIR) + "results/contours_accu-" + str_id_ + ".txt";
    ...
    static void
    saveContours(const std::string &fpath, const std::vector <std::vector<std::shared_ptr < ContourView>>
    ...
    {
        cv::Mat mask;
        cv::threshold(getBevImage(), mask, cfg_.lv_grads_[level], 123, cv::THRESH_TOZERO); // mask is same type and dimension as bev_
...
//          output(cv::Rect(config.n_col_, i * config.n_row_ + i, config.n_col_, config.n_row_)));
            ...
            cm2.getContourImage(i).copyTo(output(cv::Rect(i * config.n_col_ + i, config.n_row_ + 1, config.n_col_, config.n_row_)));
        }
