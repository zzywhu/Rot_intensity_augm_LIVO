...
#define CFILTER_API __declspec(dllexport)
#else
#define CFILTER_API __declspec(dllimport)
#endif
#else
#define CFILTER_API
#endif
...
class CFILTER_API CFilter : public CloudUtility<PointT>
{
  public:
	struct idpair_t
	{
...
#pragma omp parallel for
        ...
        std::cout << "Ground: [" << cloud_ground->points.size() << " | " << cloud_ground_down->points.size() << "] Unground: [" << cloud_unground->points.size() << "].";
        ...
        {
             std::cout << "Ground segmentation done in [" << ground_seg_time.count() * 1000.0 << "] ms.";
                                          ...
                                          float dist_thre, int max_iter, float &nx, float &ny, float &nz)
    {
        ...
        cProc.plane_seg_ransac(grid_ground, dist_thre, max_iter, grid_ground_fit, grid_coeff);
...

    {
        ...
        pca_estimator.get_pc_pca_feature(cloud_in, cloud_features,
                                         tree, neighbor_searching_radius,
                                         neighbor_k, 1, pca_down_rate,
                                         use_distance_adaptive_pca, unit_distance);
...
#pragma omp section
        ...
        std::vector<int>().swap(index_with_feature);
         ...
         std::cout  << "Unground geometric feature points extracted done in [" << time_used_all.count() * 1000.0 << "] ms.";
         std::cout  << "Details: pca in [" << time_used_pca.count() * 1000.0 << "] ms, geometric feature points extracted in [" << time_used_extract_geo_pts.count() * 1000.0 << "] ms, encoding keypoints in [" << time_used_encoding_key_pts.count() * 1000.0 << "] ms, nms sharpen in [" << time_used_nms_sharping.count() * 1000.0 << "] ms, downsampling in [" << time_used_fixed_num_downsampling.count() * 1000.0 << "] ms.";
         std::cout  << "Pillar: [" << cloud_pillar->points.size() << " | " << cloud_pillar_down->points.size() << "] Beam: [" << cloud_beam->points.size() << " | " << cloud_beam_down->points.size() << "] Facade: [" << cloud_facade->points.size() << " | " << cloud_facade_down->points.size() << "] Roof: [" << cloud_roof->points.size() << " | "
