...
namespace GlobalRegistration
{
    ...
    Match4PCSBase::Match4PCSBase(const Match4PCSOptions &options, const Utils::Logger &logger
#ifdef SUPER4PCS_USE_OPENMP
            , const int omp_nthread_congruent
#endif
    )
            : number_of_trials_(0), max_base_diameter_(-1), P_mean_distance_(1.0), best_LCP_(0.0), options_(options), randomGenerator_(options.randomSeed), logger_(logger)
#ifdef SUPER4PCS_USE_OPENMP
