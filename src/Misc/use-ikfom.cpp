...
Eigen::Matrix<double, 24, 1> get_f(state_ikfom &s, const input_ikfom &in)
{
	...
	vect3 omega;
	in.gyro.boxminus(omega, s.bg);//w-bg
	vect3 a_inertial = s.rot * (in.acc-s.ba); //Rwi*(a-ba)
	for(int i = 0; i < 3; i++ ){
		res(i) = s.vel[i];//v
		res(i + 3) =  omega[i]; //w-bg
		res(i + 12) = a_inertial[i] + s.grav[i];// Rwi*(a-ba)+g
	}
	return res;
}
...
Eigen::Matrix<double, 24, 23> df_dx(state_ikfom &s, const input_ikfom &in)
{
	...
	//std::cout<<std::fixed<<std::setprecision(16)<<cov<<std::endl;
	return cov;
}
...
vect3 SO3ToEuler(const MTK::SO3<double> &orient)
{
