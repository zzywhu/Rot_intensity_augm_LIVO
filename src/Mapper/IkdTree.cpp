...
void KD_TREE::Nearest_Search(PointType point, int k_nearest, PointVector &Nearest_Points, std::vector<float> &Point_Distance, double max_dist)
{
...
PointVector KD_TREE::Add_Points(PointVector &PointToAdd, bool downsample_on)
{
    PointVector pointsAdded;
        ...
        if (downsample_switch)
        {
            ...
            if (Rebuild_Ptr == nullptr || *Rebuild_Ptr != Root_Node)
            {
                if (Downsample_Storage.size() > 1 || same_point(PointToAdd[i], downsample_result))
                {
                    ...
                    pointsAdded.emplace_back(downsample_result);
                    tmp_counter++;
                }
            }
            else
