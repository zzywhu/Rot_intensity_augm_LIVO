        ...
        : time_delay_IMU_wtr_Lidar(0.0), time_lag_1(0.0), time_lag_2(0.0), lag_IMU_wtr_Lidar(0)
{
    ...
    fout_before_filt_IMU.open(FILE_DIR("IMU_before_filter.txt"), std::ios::out);
    fout_before_filt_Lidar.open(FILE_DIR("Lidar_before_filter.txt"), std::ios::out);
...
void LI_Init::set_IMU_state(const std::deque<CalibState> &IMU_states)
{
    IMU_state_group.assign(IMU_states.begin(), IMU_states.end() - 1);
}

void LI_Init::set_Lidar_state(const std::deque<CalibState> &Lidar_states)
{
    Lidar_state_group.assign(Lidar_states.begin(), Lidar_states.end() - 1);
}

void LI_Init::set_states_2nd_filter(const std::deque<CalibState> &IMU_states, const std::deque<CalibState> &Lidar_states)
{
...
void LI_Init::fout_before_filter()
{
    ...
    {
        fout_before_filt_IMU << std::setprecision(15) << it_IMU->ang_vel.transpose() << " " << it_IMU->ang_vel.norm() << " "
                             << it_IMU->linear_acc.transpose() << " " << it_IMU->timeStamp << std::endl;
    }
    for (auto it = Lidar_state_group.begin(); it != Lidar_state_group.end() - 1; it++)
    {
        fout_before_filt_Lidar << std::setprecision(15) << it->ang_vel.transpose() << " " << it->ang_vel.norm() << " "
                               << it->timeStamp << std::endl;
    }
}

void LI_Init::push_ALL_IMU_CalibState(const SensorMsgs::ImuData::ConstPtr &msg, const double &mean_acc_norm)
{
    CalibState IMUstate;
...
void LI_Init::downsample_interpolate_IMU(const double &move_start_time)
{
    ...
    std::deque<CalibState> IMU_states_all_origin;
...
void LI_Init::central_diff()
{
    ...
    for (; it_IMU_state != IMU_state_group.end() - 2; it_IMU_state++)
    {
        ...
        fout_IMU_meas << std::setprecision(12) << it_IMU_state->ang_vel.transpose() << " " << it_IMU_state->ang_vel.norm()
                      << " "
                      <<
...
void LI_Init::Butter_filt(const std::deque<CalibState> &signal_in, std:: deque<CalibState> &signal_out)
{
    ...
    std::deque<CalibState> sig_extended(signal_in);
    ...
    std::deque<CalibState> sig_out(sig_extended);
...
void LI_Init::zero_phase_filt(const std::deque<CalibState> &signal_in, std::deque<CalibState> &signal_out)
{
...
void LI_Init::solve_Rot_bias_gyro(double &timediff_imu_wrt_lidar)
{
    ...
    std::cout << "Using LIO: SUBTRACT this value from IMU timestamp" << std::endl
         << "           or ADD this value to LiDAR timestamp." << std::endl << std::endl;
    ...
    for (int i = 0; i < Lidar_state_group.size(); i++)
    {
        fout_after_rot << std::setprecision(12) << (Rot_Lidar_wrt_IMU * Lidar_state_group[i].ang_vel + gyro_bias).transpose()
...
void LI_Init::solve_trans_biasacc_grav()
{
    ...
    for (int i = 0; i < IMU_state_group.size(); i++)
    {
        ...
        fout_acc_cost << std::setprecision(10) << acc_I.transpose() << " " << acc_L.transpose() << " "
                      << IMU_state_group[i].timeStamp << " " << Lidar_state_group[i].timeStamp << std::endl;
    }
...
void LI_Init::normalize_acc(std::deque<CalibState> &signal_in)
{
    V3D mean_acc(0, 0, 0);
...
void LI_Init::print_initialization_result(double &time_L_I, M3D &R_L_I, V3D &p_L_I, V3D &bias_g, V3D &bias_a, V3D gravity)
{
    ...
    std::cout << std::setprecision(6)
         << "Rotation LiDAR to IMU    = " << RotMtoEuler(R_L_I).transpose() * 57.3 << " deg" << std::endl;
    printf(BOLDCYAN "[Init Result] " RESET);
    ...
    printf(BOLDCYAN "[Init Result] " RESET);
    std::cout << "Gravity in World Frame   = " << gravity.transpose() << " m/s^2" << std::endl << std::endl;
}

void LI_Init::plot_result()
{
    std::vector<std::vector<double>> IMU_omg(3), IMU_acc(3), IMU_ang_acc(3), Lidar_omg(3), Lidar_acc(3), Lidar_ang_acc(3);
...
void LI_Init::outputPlot(const std::string& filePath)
{
    std::vector<std::vector<double>> IMU_omg(3), IMU_acc(3), IMU_ang_acc(3), Lidar_omg(3), Lidar_acc(3), Lidar_ang_acc(3);
    for (auto it_IMU_state = IMU_state_group.begin(); it_IMU_state != IMU_state_group.end() - 1; it_IMU_state++)
    {
...
void LI_Init::outputErrorPlot(const std::string& filePath)
{
    std::vector<std::vector<double>> omgErrs(3), accErrs(3), angAccErrs(3);
    std::vector<Eigen::Vector3d> omgErrVec,accErrVec,angAccErrVec;
    auto it_IMU_state = IMU_state_group.begin();
    auto it_Lidar_state = Lidar_state_group.begin();
    for (; it_IMU_state != IMU_state_group.end() - 1; it_IMU_state++,it_Lidar_state++)
    {
       // std::cout<<std::setprecision(6)<<std::fixed<<"Imu / lidar timestamp:"<<it_IMU_state->timeStamp<<"/"<<it_Lidar_state->timeStamp<<std::endl;
        for (int i = 0; i < 3; i++)
        {
            omgErrs[i].push_back(abs(it_IMU_state->ang_vel[i] - it_Lidar_state->ang_vel[i]));
            accErrs[i].push_back(abs(it_IMU_state->linear_acc[i]+Grav_L0[i] - it_Lidar_state->linear_acc[i]));
            angAccErrs[i].push_back(abs(it_IMU_state->ang_acc[i] - it_Lidar_state->ang_acc[i]));
        }
//        Eigen::Vector3d omgErrsEig;
//        omgErrsEig<<omgErrs[0].back(),omgErrs[1].back(),omgErrs[2].back();
//        Eigen::Vector3d accErrEig;
//        accErrEig<<accErrs[0].back(),accErrs[1].back(),accErrs[2].back();
//        Eigen::Vector3d angAccEig;
//        angAccEig<<angAccErrs[0].back(),angAccErrs[1].back(),angAccErrs[2].back();

        omgErrVec.emplace_back(omgErrs[0].back(),omgErrs[1].back(),omgErrs[2].back());
        angAccErrVec.emplace_back(angAccErrs[0].back(),angAccErrs[1].back(),angAccErrs[2].back());
        accErrVec.emplace_back(accErrs[0].back(),accErrs[1].back(),accErrs[2].back());
    }
    std::ofstream errOfs = std::ofstream(std::string(ROOT_DIR) + "result/ErrStatistic.txt", std::ios::trunc | std::ios::in);
    double errorMax=FLT_MIN;
    double errorMin=FLT_MAX;
    double errorMean=0;
    double errorRMSE=0;
    double errorMedian=0;
    double totalError=0;
    double totalError2=0;
    std::vector<double> errVector;
    for(auto& errVec:omgErrVec)
    {
        double err2=errVec[0]*errVec[0]+errVec[1]*errVec[1]+errVec[2]*errVec[2];
        ...
        if(err<errorMin)
            errorMin=err;
        errVector.push_back(err);
    }
    errorMean=totalError/omgErrVec.size();
    errorRMSE=sqrt(totalError2/omgErrVec.size());
    std::sort(errVector.begin(),errVector.end());
    errOfs<<"Angle velocity Error statistic"<<std::endl;
    errOfs<<"mean error:"<<errorMean<<std::endl;
    errOfs<<"error RMSE:"<<errorRMSE<<std::endl;
    errOfs<<"median error:"<<errVector[(int)(errVector.size()/2)]<<std::endl;
    errOfs<<"max error:"<<errorMax<<std::endl;
    errOfs<<"min error:"<<errorMin<<std::endl<<std::endl;
    ...
    errorMean=totalError/angAccErrVec.size();
    errorRMSE=sqrt(totalError2/angAccErrVec.size());
    std::sort(errVector.begin(),errVector.end());
    errOfs<<"Angle acc Error statistic"<<std::endl;
    errOfs<<"mean error:"<<errorMean<<std::endl;
    errOfs<<"error RMSE:"<<errorRMSE<<std::endl;
    errOfs<<"median error:"<<errVector[(int)(errVector.size()/2)]<<std::endl;
    errOfs<<"max error:"<<errorMax<<std::endl;
    ...
    for(auto& errVec:accErrVec)
    {
        ...
        if(err<errorMin)
            errorMin=err;
        errVector.push_back(err);
        //std::cout<<"Err:"<<err<<" Err Vec: "<<errVec[0]<<","<<errVec[1]<<","<<errVec[2]<<std::endl;
    }

    errorMean=totalError/accErrVec.size();
    errorRMSE=sqrt(totalError2/accErrVec.size());
    std::sort(errVector.begin(),errVector.end());
    errOfs<<"acc Error statistic"<<std::endl;
    errOfs<<"mean error:"<<errorMean<<std::endl;
    errOfs<<"error RMSE:"<<errorRMSE<<std::endl;
    errOfs<<"median error:"<<errVector[(int)(errVector.size()/2)]<<std::endl;
    errOfs<<"max error:"<<errorMax<<std::endl;
